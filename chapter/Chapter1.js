window.Chapter1 = {
    ...
    // 初始化章节内容
    initialize: function() {
        MusicManager.playBGM('chapter1'); 
        // 设置章节标题
        setChapterTitle('第一章&emsp;藤蔓森林');
        changeBackground('Chapter1');
        // 设置初始内容
        const initialHTML = `
        ...
        if (choice === 'd1') {
            ...
            if (result) {
                ...
                content.innerHTML = `
                    ...
                    <p>战斗胜利！你获得了战斗力加成：${reward.power}，金币：${reward.gold}</p>
                ...
                if (ItemSystem.hasItem('绿宝石')) {
                    ...
                    <p>一地狼藉。狼的尸体四分五裂</p>
                    <p>你收到了系统提示：你获得了金币：${reward.gold}</p>
            ...
            content.innerHTML = `
                <p>这时，一团黑雾从一只狼的尸体中涌出</p>
                <p>正当你迷惑之时，指针突然躁动起来</p>
                <p>将那股黑雾吸收殆尽，指针中传来声音</p>
                <p>“恭喜你获得你的第一个战灵”</p>
                <p>“他可以为你战斗，且不会死去”</p>
                <p>“除非你自己放弃它，妥善利用吧”</p>
    ...
    updateData: function(choice) {        
        switch(choice) {
            case 'd112': 
                removeItem('打火石');
                ItemSystem.addItem('简易长矛');
                break;
            case 'd113': 
                ItemSystem.addItem('生锈短刀');
                break;
            case 'd1121':
                characterData.hp += 40;
                break;
